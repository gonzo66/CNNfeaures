/*
This program gets a video name as a parameter. Images are sampled every 30 frames.
For each sampled frame, the best 200 object proposals are obtained, and the CNN are calculated.
Layer 5, 6 and predicion layer are saved in disk
*/
#include "kyheader.h"
            ...
            for (int j = 0; j < MMAX_PROPOSALS; j++)
            {
                ...
                //{
                   ...
                   //     resize(frameFloat, inputsImg,  inputsImg.size() );
                    //else
                        resize(frameFloat(window), inputsImg,  inputsImg.size() );
